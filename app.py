...
def identify_face(face_array):
    ...
    try:
        ...
        print(f"[DEBUG] identify_face: Raw prediction result: {predicted_person}")
        
        # Check if predicted person is actually registered
        if '$' in predicted_person:
            # Extract user ID from prediction (format: Name$ID$Section)
            ...
            if len(parts) >= 2:
                ...
                if predicted_user_id in registered_users:
                    ...
                    print(f"[WARNING] identify_face: Person identified but not in registered users list: {predicted_person}")
...
def attendancebtn():
            ...
            try:
                ...
                try:
                    ...
                    if identified_person == "Unknown":
                        print(f"[INFO] attendancebtn: Face rejected as 'Unknown' - not in registered users list")
                    else:
                        print(f"[INFO] attendancebtn: Face identified and verified: {identified_person}")
                        
                except Exception as hog_error:
                    print(f"[ERROR] attendancebtn: HOG feature extraction failed: {hog_error}")
    ...
    return render_template('Attendance.html', names=names, rolls=rolls, sec=sec, times=times, dates=dates, reg=reg, l=l,
                           datetoday2=datetoday2)
...
def attendancelistdate():
    ...
    if start_date and not end_date:
        ...
        cur.execute("""
            ...
            ORDER BY a.time ASC
        """, (start_date,))
        mess = f"Attendance on {start_date}"

    elif start_date and end_date:
        # Case 2: Range
        if start_date > end_date:
            return render_template('AttendanceList.html', names=[], rolls=[], sec=[], times=[], dates=[], reg=[], l=0,
                                   mess="❌ Invalid range: Start date is after end date")
        cur.execute("""
            SELECT a.name, a.id, a.section, a.time,
                   COALESCE(s.status, 'Unknown') AS status
            FROM attendance a
            LEFT JOIN student s ON a.id = s.id
            WHERE DATE(a.time) BETWEEN %s AND %s
            ORDER BY a.time ASC
        """, (start_date, end_date))
        mess = f"Attendance from {start_date} to {end_date}"

    else:
        return render_template('AttendanceList.html', names=[], rolls=[], sec=[], times=[], dates=[], reg=[], l=0,
                               mess="⚠ Please select at least a start date")
